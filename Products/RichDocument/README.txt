RichDocument
  ...
  Known Issues and Potential Improvements
    ...
    o The templates use presentational markup and style attributes in some
    places and could do with a cleanup.

    o It's not possible to upload images while the object is still in the
    portal_factory. If the object is in the factory while it is being edited
    for the first time and the user uploads an image, it will be instantiated
    into the target folder first.

    o The image and attachment controls are submit buttons, managed via
    ...
    steal the default button status - if the user presses Enter in the form,
    they may get a (benevolent) portal error message telling them they must
    upload an image.

    o The only re-ordering support provided directly by the manager widgets
